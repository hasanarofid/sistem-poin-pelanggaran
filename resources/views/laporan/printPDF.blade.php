<div style="text-align: center;">
    <table style="margin: 0 auto; width: 100%; margin-bottom: 30px;">
        <tr>
            <td colspan="7" style="text-align: center; vertical-align: middle; font-size: 30px; font-weight: bold;">
               Laporan Pelanggaran
            </td>
        </tr>
        <tr>
            ...
            @foreach ($group as $i => $item)
                <tr>
                    @if ($i == 0)
                        ...
                        <td rowspan="{{ $rowspan }}" style="vertical-align: middle; border: 1px solid #e5e7eb;">{{ $item->nama }}</td>
                        <td rowspan="{{ $rowspan }}" style="vertical-align: middle; border: 1px solid #e5e7eb;">
                            {{ $item->nama_kelas . ' (' . $item->subkelas . ')' }}
                        </td>
                    @endif
                    <td style="vertical-align: middle; border: 1px solid #e5e7eb;">{{ $item->nama_pelanggaran }}</td>
                    <td style="vertical-align: middle; border: 1px solid #e5e7eb;">{{ $item->keterangan }}</td>
                    <td style="text-align: center; vertical-align: middle; border: 1px solid #e5e7eb;">{{ $item->poin }}</td>
                    @if ($i == 0)
                        <td rowspan="{{ $rowspan }}" style="vertical-align: middle; border: 1px solid #e5e7eb;">{{ $item->pelapor }}</td>
                    @endif
                </tr>
            @endforeach
        @empty
